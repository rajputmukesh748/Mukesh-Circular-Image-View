package com.mukesh.mukeshcircleimageview
...
/**
 * OWNER {Mukesh Rajput}
 * This is create for circular image view
 * in android xml layout with border color outline
 * or without outline or shadow
 * */

class MukeshCircleImageView : AppCompatImageView {
    ...
    /**
     * Call a constructor with
     * custom attribute or styles
     * */
    @JvmOverloads
    constructor(context: Context, attrs: AttributeSet?, defStyle: Int = 0) : super(
        ...
        val a =
            context.obtainStyledAttributes(attrs, R.styleable.MukeshCircleImageView, defStyle, 0)
        mBorderWidth = a.getDimensionPixelSize(
            R.styleable.MukeshCircleImageView_miv_border_width,
            DEFAULT_BORDER_WIDTH
        )
        mBorderColor =
            a.getColor(R.styleable.MukeshCircleImageView_miv_border_color, DEFAULT_BORDER_COLOR)
        mBorderOverlay =
            a.getBoolean(
                R.styleable.MukeshCircleImageView_miv_border_overlay,
                DEFAULT_BORDER_OVERLAY
            )
        mCircleBackgroundColor = a.getColor(
            R.styleable.MukeshCircleImageView_miv_circle_background_color,
    ...
    override fun setImageAlpha(alpha: Int) {
        ...
        // This might be called during AppCompactImageView construction before
    ...
    private fun inTouchableArea(x: Float, y: Float): Boolean {
        return if (mBorderRect.isEmpty) {
            true
        } else (x - mBorderRect.centerX().toDouble()).pow(2.0) + Math.pow(
